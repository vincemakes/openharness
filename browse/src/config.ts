 ...
 *   2. git rev-parse --show-toplevel → projectDir/.openharness-browse/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    stateDir = path.join(projectDir, '.openharness-browse');
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure state dir is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.openharness-browse\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.openharness-browse/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
...
/**
 * Derive a slug from the git remote origin URL.
 */
